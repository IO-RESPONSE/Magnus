...
/* Roadmap 5a-4 (fixed alongside the connection pool, though really a
 * pre-existing 5a-1 gap the pool's own heavier concurrent testing is
 * what actually exposed it): bounds how long a FastCGI attempt may
 * spend connecting to, or waiting on, an upstream, mirroring
 * MAGNUS_PROXY_CONNECT_TIMEOUT_SECONDS/MAGNUS_PROXY_READ_TIMEOUT_
 * SECONDS exactly (same values, dedicated constants for the same
 * "separate everything, identical shape" reasoning every other
 * FastCGI-specific constant in this file already follows). Before
 * this, fastcgi_connect_started/fastcgi_last_activity were tracked but
 * never actually checked against anything -- a connect stalled behind
 ...
 * than ever answering a clean 504. See magnus_fastcgi_expire()'s own
 * doc comment for why, unlike magnus_proxy dispatch, no separate
 * "response already started, must abort" case exists here to handle
 * differently. */
#define MAGNUS_FASTCGI_CONNECT_TIMEOUT_SECONDS \
    MAGNUS_PROXY_CONNECT_TIMEOUT_SECONDS
#define MAGNUS_FASTCGI_READ_TIMEOUT_SECONDS MAGNUS_PROXY_READ_TIMEOUT_SECONDS
/* Roadmap 5b-1 (SCGI dispatch): unlike FastCGI's own 5a-1, connect/read
 * timeout enforcement is included from this very first cut rather than
 * a later increment -- the exact gap MAGNUS_FASTCGI_CONNECT_TIMEOUT_
 * SECONDS's own doc comment just above describes (a stalled or silent
 * upstream hanging the client indefinitely) is already a known, real
 * failure mode in this codebase, not a hypothetical one, so there is
 * no reason to knowingly reintroduce it into a second protocol path
 * only to fix it again later. Retry and connection pooling are still
 * deliberately deferred -- those are genuine capability layers, not a
 * safety net every dispatch path needs unconditionally the way a
 * timeout is. */
...
typedef struct {
     ...
     * 2a-10) -- the one case try_compress/proxy_compress_pending's own
     * MAGNUS_COMPRESSION_MAX_SIZE bound excludes: a response too large
     * to buffer whole before compressing once. Unlike proxy_compress_
     * capture (buffer everything, then compress in one shot), this
     * compresses incrementally as bytes arrive from upstream --
     * response headers (Content-Encoding/Vary, no Content-Length,
     * Connection: close, exactly like 2a-7's own static-file streaming
     * decision) go out immediately rather than being deferred, and
     * proxy_buffer/_length/_sent (the same three fields the ordinary
     * uncompressed relay already uses) become the pending-input queue
     * magnus_proxy_flush()'s own streaming-compress block feeds the
     * compressor from, instead of writing them to the client raw --
     * see that block's own comment. proxy_stream_compress_outbuf is
     * malloc()'d (MAGNUS_STREAM_COMPRESS_CHUNK, same as the static-file
     * streaming path's own stream_compress_outbuf) only once eligibility
     * is confirmed, not kept around unconditionally on every connection
     * the way the small, fixed-size proxy_buffer already is. */
    magnus_stream_compressor_t *proxy_stream_compress;
     ...
     * and this codebase's own deliberate scope (HTTP/1.1 only -- no
     * session affinity/caching yet, the same narrow starting point
     * every other upstream-protocol dispatch in this codebase began
     * from before later increments layered those on; roadmap 5a-2
     * lifted the original 5a-1 GET-only, no-request-body restriction --
     * any method/body magnus_begin_body()/magnus_continue_body()
     * already buffered generically ahead of dispatch is relayed the
     * same way proxy dispatch's own body relay is; roadmap 5a-3 added
     * retry against a different endpoint on any upstream-side failure,
     * up to MAGNUS_FASTCGI_MAX_ATTEMPTS -- see magnus_fastcgi_connect_
     ...
     * failed() is limited to; roadmap 5a-4 added an idle-connection
     * pool, magnus_fastcgi_pool[], mirroring magnus_upstream_pool[]'s
     * own shape via a dedicated magnus_fastcgi_pool_checkout()/
     * _checkin()/_expire_idle()/_close_all() quartet -- see that
     * array's own doc comment on why it is not shared with the ordinary
     * proxy pool). Kept entirely
    ...
    /* Roadmap 5a-4: how many requests the current fastcgi_upstream_fd
     * has already served -- 0 for a fresh connection, or whatever
     * magnus_fastcgi_pool_checkout() reports for a reused one. Passed
     * to magnus_fastcgi_pool_checkin() (incremented by one, for this
     * request) on a clean completion, and to close()/discard it
     * outright once MAGNUS_POOL_MAX_REQUESTS_PER_CONNECTION is reached
     * -- mirrors proxy_upstream_requests_served's own identical role. */
    unsigned fastcgi_upstream_requests_served;
     ...
     * relays to an SCGI application server -- see src/magnus_scgi.h for
     * the netstring-framing helpers this uses, and magnus_scgi_pick_
     * and_start()'s own doc comment in magnus.c for the full request/
     * response shape. Kept entirely separate from the fastcgi_* /
     * proxy_* fields above (a categorically different upstream
     * protocol from either, the same reasoning gRPC's and FastCGI's
     * own dedicated field sets already established) rather than
     * shared, even though SCGI's own response shape is close enough to
     * FastCGI's own CGI-translated one to reuse magnus_fastcgi_find_
     * body()/magnus_fastcgi_translate_headers() directly for that one
     * piece (see magnus_scgi.h's own top comment on why that reuse is
     * deliberate, not an oversight). Deliberately narrower than
     * fastcgi_active's own current scope: no retry, no connection
     * pooling, no session affinity yet -- see MAGNUS_ROUTE_ACTION_SCGI's
     * own doc comment (magnus_route.h) for why request-body support
     * itself is *not* similarly deferred, unlike FastCGI's own original
     * 5a-1 cut. */
    bool scgi_active;
    int scgi_upstream_fd;
    bool scgi_upstream_connected;
    size_t scgi_endpoint_index;
    bool scgi_endpoint_counted;
    time_t scgi_connect_started;
    time_t scgi_last_activity;
    char scgi_request_id[33];
    char scgi_log_method[8];
    char scgi_log_target[256];
    bool scgi_client_wants_close;
    /* The whole outbound byte stream (netstring-framed header block +
     * raw request body) is built once, up front, at dispatch time --
     * mirrors fastcgi_out's own identical shape/rationale. */
    unsigned char *scgi_out;
    size_t scgi_out_length;
    size_t scgi_out_sent;
    /* Raw response bytes accumulate here, verbatim, exactly as they
     * arrive -- unlike fastcgi_stdout (which is only ever fed content
     * already unwrapped from STDOUT records by magnus_fastcgi_consume_
     * records()), SCGI's own response has no framing to unwrap in the
     * first place, so recv()'d bytes land here directly. Translated
     * into a real HTTP/1.1 response only once the upstream closes the
     * connection (magnus_scgi_finish()) -- SCGI has no FCGI_END_
     * REQUEST-equivalent marker, so EOF is the only completion signal
     * the protocol itself ever provides (see magnus_scgi_handle_
     * upstream()'s own doc comment). Buffered whole rather than
     * streamed, same "narrow first cut" reasoning fastcgi_stdout's own
     * comment already documents. */
    char *scgi_stdout;
    size_t scgi_stdout_length;
    size_t scgi_stdout_capacity;
    /* Populated once, by magnus_scgi_finish(), from the translated CGI
     * header block within scgi_stdout above -- mirrors fastcgi_
     * response_headers/fastcgi_body_offset/fastcgi_body_sent's own
     * identical shape and rationale. */
    char *scgi_response_headers;
    size_t scgi_response_headers_length;
...
/* SCGI dispatch (roadmap 5b-1) -- its own dedicated cluster, the same
 * "not a shared namespace" reasoning magnus_fastcgi_cluster's own
 * comment just above gives (and, transitively, magnus_grpc_cluster's).
 * magnus_scgi_root plays the same DOCUMENT_ROOT role magnus_fastcgi_
 * root does. */
...
/* Roadmap 5a-4: FastCGI's own idle-connection pool, kept entirely
 * separate from magnus_upstream_pool[] above -- the same reasoning
 * magnus_fastcgi_cluster's own "not a shared namespace" comment
 * already established: magnus_fastcgi_cluster has its own endpoint
 * index space (0..MAGNUS_CONFIG_MAX_FASTCGI_UPSTREAMS-1), unrelated to
 * action=proxy's own magnus_cluster endpoint indices, so sharing one
 * pool array between them would silently hand out a connection meant
 * for one protocol's endpoint N to the other protocol's own,
 * unrelated endpoint N. Reuses magnus_pool_slot_t/magnus_pool_t
 * verbatim (a plain fd+idle_since+requests_served slot has nothing
 * proxy-specific about its shape) via a dedicated
 * magnus_fastcgi_pool_checkout()/_checkin()/_expire_idle()/
 * _close_all() quartet mirroring magnus_pool_checkout() et al.
 * exactly, the same "separate everything, identical shape" precedent
 * magnus_grpc_pool[][]'s own comment already used for gRPC. */
static magnus_pool_t magnus_fastcgi_pool[MAGNUS_CONFIG_MAX_FASTCGI_UPSTREAMS];

/* See magnus_pool_checkout()'s own doc comment -- identical contract,
 * against magnus_fastcgi_pool[] instead. Requires the app server to
 * have honored FCGI_KEEP_CONN (magnus_fastcgi_build_request() always
 * requests it); one that does not (or has since closed for its own
 * reasons) is caught by the same MSG_PEEK staleness check below and
 * simply discarded, falling back to a fresh connection exactly as if
 * pooling had found nothing. */
static int
magnus_fastcgi_pool_checkout(size_t endpoint_index,
                             unsigned *out_requests_served)
{
    magnus_pool_t *pool;
    if (endpoint_index >= MAGNUS_CONFIG_MAX_FASTCGI_UPSTREAMS) return -1;
    ...
    while (pool->count > 0) {
        ...
        if (peeked > 0) {
            /* A FastCGI application server has no business sending
             * anything while idle between requests either -- same
             * distrust magnus_pool_checkout()'s own identical check
             * already applies to an HTTP/1.x upstream. */
            close(slot.fd);
            continue;
        }
...
/* See magnus_pool_checkin()'s own doc comment -- identical contract,
 * against magnus_fastcgi_pool[] instead. */
static void
magnus_fastcgi_pool_checkin(size_t endpoint_index, int fd,
                            unsigned requests_served)
{
    magnus_pool_t *pool;
    if (endpoint_index >= MAGNUS_CONFIG_MAX_FASTCGI_UPSTREAMS) {
        close(fd);
        return;
    }
    pool = &magnus_fastcgi_pool[endpoint_index];
...
/* See magnus_pool_expire_idle()'s own doc comment -- identical
 * contract, against magnus_fastcgi_pool[] instead. */
static void
magnus_fastcgi_pool_expire_idle(time_t now)
{
    for (size_t endpoint = 0; endpoint < MAGNUS_CONFIG_MAX_FASTCGI_UPSTREAMS;
         endpoint++) {
        magnus_pool_t *pool = &magnus_fastcgi_pool[endpoint];
...
/* See magnus_pool_close_all()'s own doc comment -- identical contract,
 * against magnus_fastcgi_pool[] instead. */
static void
magnus_fastcgi_pool_close_all(void)
{
    for (size_t endpoint = 0; endpoint < MAGNUS_CONFIG_MAX_FASTCGI_UPSTREAMS;
         endpoint++) {
        magnus_pool_t *pool = &magnus_fastcgi_pool[endpoint];
...
magnus_close_connection(int epoll_fd, magnus_connection_t *connection)
{
    ...
    /* Roadmap 2a-10: same "a client/upstream can disconnect mid-stream"
     * tolerance as stream_compress_end() above -- see that call's own
     * comment. */
    ...
    /* SCGI dispatch (roadmap 5b-1): same tolerance as the FastCGI block
     * just above. */
    free(connection->scgi_out);
    free(connection->scgi_stdout);
    free(connection->scgi_response_headers);
    if (connection->scgi_endpoint_counted) {
    ...
    if (connection->scgi_upstream_fd >= 0) {
        epoll_ctl(epoll_fd, EPOLL_CTL_DEL, connection->scgi_upstream_fd,
                 NULL);
...
magnus_proxy_teardown_upstream(int epoll_fd, magnus_connection_t *connection)
{
    ...
    /* Roadmap 2a-10: same "a retry must not carry a failed/finished
     * attempt's partial state forward" reasoning as proxy_compress_
     * capture just above -- a successful, streamed completion has
     * already finished and freed this compressor from within magnus_
     * proxy_flush()'s own streaming-compress block before ever reaching
     * here (magnus_stream_compress_end()/free() below are therefore
     * both already-NULL no-ops on that path, same tolerance every other
     * teardown call in this codebase relies on). */
    magnus_stream_compress_end(connection->proxy_stream_compress);
    connection->proxy_stream_compress = NULL;
    free(connection->proxy_stream_compress_outbuf);
    connection->proxy_stream_compress_outbuf = NULL;
...
{
    /* Hoisted to function scope (roadmap 2a-10): the streaming-compress
     * block below needs this exact same "has every upstream body byte
     * this response will ever have already arrived" fact to know when
     * to finish=true its last magnus_stream_compress_step() call, same
     * as the pre-existing "response complete" block further down that
     * originally computed it locally -- see that block's own comment,
     * unchanged other than no longer redeclaring this. */
    ...
    /* Roadmap 2a-10: streaming proxy dispatch response compression --
     * entered either while proxy_stream_compress is still live, or (an
     * EAGAIN on the very last write below left some already-produced
     * bytes undrained) while it has already finished but
     * proxy_stream_compress_outbuf itself has not yet fully drained.
     * Unlike the static-file streaming path (2a-7/2a-8/2a-9), input is
     * never pulled on demand here (there is no pread() to call for
     * more) -- it only ever arrives pushed, asynchronously, by
     * magnus_handle_upstream()'s own recv() off the upstream socket,
     * into proxy_buffer/_length exactly like the ordinary uncompressed
     * relay already uses those same three fields (proxy_buffer_sent
     * here plays the same "how much of the current chunk has been
     * consumed" role that field already plays there). So when this
     * function is entered with nothing left to consume and upstream
     * has not signaled complete_by_length/proxy_eof yet, it simply
     * re-arms the upstream fd for reading and returns -- exactly the
     * same wait this same situation already gets on the uncompressed
     * path just below -- rather than looping: there is nothing for it
     * to do until magnus_handle_upstream() calls back in with more. */
    if (connection->proxy_stream_compress != NULL
        || connection->proxy_stream_compress_outbuf_sent
           < connection->proxy_stream_compress_outbuf_length) {
        for (;;) {
            while (connection->proxy_stream_compress_outbuf_sent
                   < connection->proxy_stream_compress_outbuf_length) {
                ssize_t sent = magnus_socket_write(connection,
            ...
            /* Fully drained. Nothing left to feed the compressor either
             * (it already reported done, on a prior iteration/call) --
             * fall out to the ordinary "response complete" reasoning
             * below, exactly like the whole-buffered proxy_compressed_
             * body case already does once IT finishes draining. */
            if (connection->proxy_stream_compress == NULL) break;

            if (connection->proxy_buffer_sent == connection->proxy_buffer_length
...

            {
                ...
                if (connection->proxy_buffer_sent
                    == connection->proxy_buffer_length) {
                ...
                if (done) {
                     ...
                     * call cannot be trusted to ever produce output at
                     * all, let alone signal done, for zstd/Brotli) means
                     * this loop must never treat a zero-progress call as
                     * a reason to stop retrying on its own -- but done
                     * becoming true is always real forward progress
                     * (finish=true was passed, and every one of the
                     * three encoders' own "am I finished" flags only
                     * ever turns true together with actually having
                     * flushed everything), so looping back to the top to
                     * drain this final chunk and then break is always
                     ...
                     * leave stale, would-be-relayed-raw bytes sitting in
                     * proxy_buffer for the plain-relay code just below
                     * this whole block to mistake for an ordinary
                     * uncompressed response if that assumption is ever
                     * wrong. */
                    connection->proxy_buffer_length = 0;
                    connection->proxy_buffer_sent = 0;
                }
...
magnus_proxy_receive_headers(int epoll_fd, magnus_connection_t *connection)
{
     ...
     * anything past MAX_SIZE falls to try_stream_compress just below
     * instead (roadmap 2a-10) rather than being simply excluded the way
     * it still was through 2a-9. A response with no unambiguous
     * Content-Length is also never eligible for either mode: "read
     * exactly this many bytes from upstream, compressing as they
     * arrive (whole-buffered or streamed)" has no sensible answer for
     * a chunked/close-delimited upstream body. */
    ...
    /* Roadmap 2a-10: the streaming analogue of try_compress, for exactly
     * the one case its own MAGNUS_COMPRESSION_MAX_SIZE bound excludes --
     * a response too large to buffer whole before compressing once.
     * MAGNUS_COMPRESSION_MIN_SIZE is not checked: anything past
     * MAX_SIZE is already far past MIN_SIZE. */
    bool try_stream_compress = connection->proxy_accept_encoding != MAGNUS_ENCODING_NONE
        && info.status == 200
        && !info.has_content_encoding && info.has_content_length
        && info.content_length > MAGNUS_COMPRESSION_MAX_SIZE
        && magnus_content_type_compressible(info.content_type);
    ...
    if (try_compress) {
        ...
        connection->proxy_compress_capture_length = 0;
        connection->proxy_compress_capture_capacity = 0;
    } else if (try_stream_compress) {
        /* Unlike try_compress, nothing here is deferred: a streamed
         * response's real compressed length can never be known ahead of
         * time any more than a streamed static file's own can (roadmap
         * 2a-7), so headers go out right now instead of waiting for a
         * body that may never finish arriving. A genuine allocation
         * failure (the compressor itself, or its output buffer) simply
         * declines streaming -- proxy_stream_compress stays NULL, and
         * the plain uncompressed relay below runs exactly as if
         * try_stream_compress had been false, the same graceful
         * degradation magnus_proxy_finish_compression() already has for
         * try_compress's own encoder-allocation-failure case. */
        connection->proxy_stream_compress
    ...
    if (connection->proxy_stream_compress != NULL) {
        char stream_sanitize_scratch[MAGNUS_PROXY_HEADER_LIMIT + 1];
        char stream_sanitized[MAGNUS_PROXY_SANITIZED_LIMIT];
        magnus_proxy_response_info_t stream_info;
        int stream_sanitized_length;

        memcpy(stream_sanitize_scratch, header_copy, header_length + 1);
        stream_sanitized_length = magnus_proxy_sanitize_response_headers(
            stream_sanitize_scratch, header_length, stream_sanitized,
            sizeof(stream_sanitized),
            connection->proxy_issue_affinity_cookie
                ? connection->proxy_affinity_key : NULL,
            connection->proxy_client_wants_close,
        ...
        connection->proxy_header_out = malloc((size_t) stream_sanitized_length);
        if (connection->proxy_header_out == NULL) {
            magnus_stream_compress_end(connection->proxy_stream_compress);
            connection->proxy_stream_compress = NULL;
            free(connection->proxy_stream_compress_outbuf);
        ...
        memcpy(connection->proxy_header_out, stream_sanitized,
...
struct magnus_h2_stream {
    ...
    size_t cache_capture_capacity;
    bool cache_capture_overflowed;
    /* Proxy dispatch response compression, HTTP/2 (roadmap 2a-3) -- the
     * h2 analogue of magnus_connection_t's own identically-purposed
     * fields (see that struct's own comment for the full rationale of
     * why compression needs its own gate, unlike cache_capture's pure
     * side observation). No proxy_accept_gzip field needed here the way
     * HTTP/1.1's own connection struct needs one: stream->parsed (this
     * struct's own persistent copy of the client's request, unlike
     * HTTP/1.1's stack-local one) survives for the stream's whole
     * lifetime, so magnus_h2_proxy_receive_headers() reads Accept-
     * Encoding from it directly instead. No dedicated compressed-body
     * field either: stream->io_buffer is already a generically
     * reassignable heap pointer (see its own comment below), reused
     * directly for the compressed body the exact same way the static-
     ...
     * 2a-11) -- the h2 analogue of magnus_connection_t's own 2a-10
     * fields, distinct from both compress_pending/compress_capture just
     * above (the buffer-then-compress path) and stream_compress/
     * stream_compress_inbuf below (2a-8's own static-file streaming --
     * different feature, different lifecycle, deliberately not shared
     * field names even though at most one of the three is ever active
     * for a given stream). Unlike HTTP/1.1's connection->proxy_buffer
     * (reused directly as the compressor's pending-input queue),
     * stream->io_buffer here is repurposed as the compressed *output*
     * queue instead -- magnus_h2_read_io_buffer() already knows how to
     * pull from it and report DEFERRED/EOF correctly, so reusing it
     * avoids a second read_callback entirely, but that means raw,
     * not-yet-compressed bytes recv()'d from upstream need a *different*
     * home: proxy_stream_compress_inbuf/_length/_sent, a dedicated
     * MAGNUS_PROXY_BUFFER-sized staging buffer, fills that role. */
    magnus_stream_compressor_t *proxy_stream_compress;
    unsigned char *proxy_stream_compress_inbuf;
    size_t proxy_stream_compress_inbuf_length;
    size_t proxy_stream_compress_inbuf_sent;
...
magnus_h2_stream_teardown_upstream(struct magnus_h2_stream *stream)
{
    ...
    /* Same reasoning as cache_capture just above, for the identical
     * "a retry/abnormal ending must not carry a failed attempt's
     * partial state forward" reason (roadmap 2a-3). A successful,
     * compressed completion has already handed compress_capture's
     * contents off (freed, or transferred into stream->io_buffer on the
     * gzip-allocation-failure fallback) and cleared compress_pending
     * before ever reaching here -- see magnus_h2_proxy_finish_
     * compression(). */
    stream->compress_pending = false;
    free(stream->compress_capture);
    stream->compress_capture = NULL;
    stream->compress_capture_length = 0;
    stream->compress_capture_capacity = 0;
    /* Roadmap 2a-11: same "a retry/abnormal ending must not carry a
     * failed/finished attempt's partial state forward" reasoning as
     * compress_capture just above -- a successful, streamed completion
     * has already finished and freed this compressor from within
     * magnus_h2_proxy_stream_compress_response() before ever reaching
     * here (magnus_stream_compress_end()/free() below are therefore
     * already-NULL no-ops on that path, same tolerance every other
     * teardown call in this codebase relies on). */
    magnus_stream_compress_end(stream->proxy_stream_compress);
    stream->proxy_stream_compress = NULL;
    free(stream->proxy_stream_compress_inbuf);
    stream->proxy_stream_compress_inbuf = NULL;
    stream->proxy_stream_compress_inbuf_length = 0;
...
magnus_h2_stream_free(struct magnus_h2_stream *stream)
{
    ...
    /* Roadmap 2a-11: proxy_stream_compress/proxy_stream_compress_inbuf
     * are NOT freed here directly (unlike stream_compress/
     * stream_compress_inbuf just above, an unrelated static-file-only
     * feature teardown_upstream() never touches) -- they are upstream-
     * attempt-scoped, exactly like cache_capture/compress_capture below,
     * so magnus_h2_stream_teardown_upstream() already frees and NULLs
     * them; freeing here too, before that call runs, would double-free
     * the still-live pointer instead of the harmless free(NULL) a
     * *post*-teardown_upstream() redundant free (like cache_capture's/
     * compress_capture's own below) would be. */
...
/* The h2 analogue of magnus_proxy_compress_capture() -- see that
 * function's own comment for the full rationale, identical here except
 * operating on stream->compress_capture* instead of
 * connection->proxy_compress_capture*. */
static bool
magnus_h2_proxy_compress_capture(struct magnus_h2_stream *stream,
                                 const char *data, size_t len)
{
    if (len == 0) return true;
    if (stream->compress_capture_length + len
        ...
        size_t new_capacity = stream->compress_capture_capacity == 0
            ? MAGNUS_PROXY_BUFFER : stream->compress_capture_capacity * 2;
        char *grown;
...
/* The h2 analogue of magnus_proxy_finish_compression() -- see that
 * function's own comment for the full rationale. Called from
 * magnus_h2_proxy_maybe_complete() the moment a compress-pending
 * stream's body is fully captured (or the upstream closed early),
 * *before* magnus_h2_proxy_submit_response() has ever been called for
 * this stream (compression is exactly why it was deferred -- see
 * magnus_h2_proxy_receive_headers()'s own comment). Unlike the HTTP/1.1
 * path, there is no separate write loop to feed afterward: nghttp2's
 * own magnus_h2_read_io_buffer() pull callback drains stream->io_buffer
 ...
 * magnus_h2_proxy_fail()'s own precondition still holds; the caller
 * handles it exactly that way). */
static int
magnus_h2_proxy_finish_compression(struct magnus_h2_stream *stream)
{
    magnus_connection_t *connection = stream->connection;
    unsigned char *compressed = NULL;
    size_t compressed_length = 0;
    ...
    if (compressed_ok) {
         ...
         * copy. */
        stream->io_buffer = stream->compress_capture;
        stream->io_length = stream->compress_capture_length;
        stream->compress_capture = NULL;
        stream->compress_capture_capacity = 0;
    }
    stream->io_sent = 0;
...
magnus_h2_proxy_maybe_complete(struct magnus_h2_stream *stream)
{
    ...
    /* Roadmap 2a-3: response_headers_submitted is still false here for
     * a compress-pending stream (magnus_h2_proxy_receive_headers() only
     * ever buffered so far, exactly like the HTTP/1.1 path's own
     * proxy_compress_pending gate -- see that comment) -- finalizing
     * calls magnus_h2_proxy_submit_response() for the first time, right
     * here, whether or not the body's own declared length was ever
     * fully reached (a truncated upstream still finalizes with whatever
     * was captured, same tolerance the pre-existing, non-compressing
     * behavior just below this block already has for that identical
     * scenario). */
    if (stream->compress_pending) {
        stream->compress_pending = false;
        if (magnus_h2_proxy_finish_compression(stream) != 0) {
            magnus_h2_proxy_fail(stream->connection, stream, "502");
            return;
        }
    }
                                ...
                                struct magnus_h2_stream *stream)
{
    ...
    /* Roadmap 2a-3: decided once, right here, the moment headers
     * (status/content-type/content-length/an existing Content-Encoding,
     ...
     * captured at dispatch time. */
    {
        ...
        bool try_compress = encoding != MAGNUS_ENCODING_NONE
            ...
            && info.content_length <= MAGNUS_COMPRESSION_MAX_SIZE
            && magnus_content_type_compressible(info.content_type);
        /* Roadmap 2a-11: the streaming analogue of try_compress, for
         * exactly the one case its own MAGNUS_COMPRESSION_MAX_SIZE
         * bound excludes -- see magnus_proxy_receive_headers()'s own
         * identical HTTP/1.1 comment. */
        bool try_stream_compress = encoding != MAGNUS_ENCODING_NONE
            && info.status == 200 && !info.has_content_encoding
            && info.has_content_length
            && info.content_length > MAGNUS_COMPRESSION_MAX_SIZE
            && magnus_content_type_compressible(info.content_type);
        ...
        stream->proxy_stream_compress = NULL;
        if (try_compress) {
            /* Defer submit_response2() until the whole body is captured
             * and compressed (see magnus_h2_proxy_maybe_complete()'s
             * own finalize block) -- stash the RAW upstream header
             * block (header_copy is still pristine: sanitize tokenized
             * its own separate scratch copy) so magnus_h2_proxy_finish_
             * compression() can sanitize it a second time, with the
             * real compressed Content-Length, once compression
            ...
            stream->compress_capture_length = 0;
            stream->compress_capture_capacity = 0;
            if (!magnus_h2_proxy_compress_capture(stream, body_start,
                    leftover)) {
                magnus_h2_proxy_connect_failed(connection, stream, "502");
                return true;
            }
            ...
            /* Unlike try_compress, nothing here is deferred -- see
             * magnus_proxy_receive_headers()'s own identical HTTP/1.1
             * comment. A genuine allocation failure (the compressor
             * itself, or its input staging buffer) simply declines
             * streaming, exactly like the HTTP/1.1 path's own identical
             * fallback: proxy_stream_compress stays NULL, and the plain
             * uncompressed relay below runs as if try_stream_compress
             * had been false. */
            stream->proxy_stream_compress
                = magnus_stream_compress_begin(encoding);
            if (stream->proxy_stream_compress != NULL) {
                stream->proxy_stream_compress_inbuf
                    = malloc(MAGNUS_PROXY_BUFFER);
                if (stream->proxy_stream_compress_inbuf == NULL) {
                    magnus_stream_compress_end(stream->proxy_stream_compress);
                    stream->proxy_stream_compress = NULL;
                }
            }
        ...
        if (stream->proxy_stream_compress != NULL) {
            /* io_buffer becomes the compressed *output* queue from here
             * on (magnus_h2_read_io_buffer() already pulls from it
             * correctly) -- this initial leftover chunk, still raw, goes
             * into the new input staging buffer instead, exactly like
             * every later magnus_h2_proxy_stream_compress_response() fill
             * does. */
            memcpy(stream->proxy_stream_compress_inbuf, body_start, leftover);
            stream->proxy_stream_compress_inbuf_length = leftover;
            stream->proxy_stream_compress_inbuf_sent = 0;
            stream->io_length = 0;
        } else if (!try_compress) {
        ...
        stream->compress_encoding = encoding; /* also read by the
            * streaming-submit block below, past this block's own scope
            * for `encoding` itself -- harmless to set even when neither
            * try_compress nor try_stream_compress fired, since
            * compress_pending/proxy_stream_compress being false/NULL
            * means nothing else ever reads it in that case. */
    }
    stream->io_sent = 0;
    ...
    magnus_cluster_result(&magnus_cluster, stream->endpoint_index, true,
                          magnus_now_ms());
    if (stream->proxy_stream_compress != NULL) {
        /* Streaming (2a-11): the real compressed length can never be
         * known ahead of time, so headers go out now, via a second
         * sanitize call using the (size_t) -2 sentinel (Content-
         * Encoding/Vary, no Content-Length -- magnus_h2_proxy_submit_
         * response() already drops the Connection header sanitize
         * would otherwise append regardless, since h2 forbids it, so
         * unlike HTTP/1.1 there is no separate "force Connection:
         * close" step needed here at all). header_copy is safe to
         * reuse (not pristine any more, but re-tokenizing it a second
         * time with the same delimiter set reproduces the identical
         * partition -- the exact same reuse magnus_h2_proxy_finish_
         * compression() already relies on for compress_raw_headers). */
        char stream_sanitized[MAGNUS_PROXY_SANITIZED_LIMIT];
        magnus_proxy_response_info_t stream_info;
        int stream_sanitized_length = magnus_proxy_sanitize_response_headers(
            header_copy, header_length, stream_sanitized,
            sizeof(stream_sanitized),
            stream->issue_affinity_cookie ? stream->affinity_key : NULL,
            true, (size_t) -2,
            magnus_encoding_name(stream->compress_encoding), &stream_info,
            NULL);
        if (stream_sanitized_length < 0) {
            magnus_h2_proxy_connect_failed(connection, stream, "502");
            return true;
        }
        magnus_h2_proxy_submit_response(connection, stream, stream_info.status,
                                        stream_sanitized);
    } else if (!stream->compress_pending) {
        magnus_h2_proxy_submit_response(connection, stream, info.status,
                                        sanitized);
    }
    magnus_requests_total++;
    if (info.status >= 500) magnus_responses_5xx++;
    ...
    if (received > 0) {
        ...
        if (stream->cache_this_response_cacheable)
            magnus_h2_proxy_cache_capture(stream, stream->io_buffer,
                                          (size_t) received);
        /* Roadmap 2a-3: redirected into the capture buffer instead of
         * being left in io_buffer for magnus_h2_read_io_buffer() to
         * pull as real response data -- see magnus_h2_proxy_receive_
         * headers()'s own comment on why nothing client-visible is
         * populated for a compress-pending stream until the whole body
         * is known. An allocation failure here is unrecoverable (no
         * uncompressed client-facing state exists to fall back to
         * either) -- magnus_h2_proxy_abort(), the same outcome any
         * other genuine mid-relay failure in this function already
         * has. */
        if (stream->compress_pending) {
            if (!magnus_h2_proxy_compress_capture(stream, stream->io_buffer,
                    (size_t) received)) {
...
/* The streaming-compress analogue of magnus_h2_proxy_stream_response()
 * (roadmap 2a-11) -- called instead of it whenever stream->
 * proxy_stream_compress is non-NULL. Structurally different from every
 * static-file streaming path this codebase has (2a-8's own
 * magnus_h2_read_stream_compressed(), a *pull*-based read_callback that
 * fetches more input itself on demand via pread()): here input only
 * ever arrives *pushed*, asynchronously, exactly like the plain h1-proxy
 * relay's own upstream recv() above, so this is a push-driven fill
 * function instead of a pull callback, mirroring magnus_proxy_flush()'s
 * own HTTP/1.1 streaming-compress block. Backpressure is on io_buffer
 * (the compressed *output* queue magnus_h2_read_io_buffer() drains, not
 * the raw upstream bytes) -- never touches anything while nghttp2 has
 * not yet pulled the previous chunk out. Loops internally exactly like
 * every other streaming-compression call site in this codebase already
 * does (roadmap 2a-9's own "zstd/Brotli don't guarantee output on every
 * call" lesson): a step() call that consumes input but produces nothing
 * simply retries -- fetching more raw input first if the staging buffer
 * is empty and more is expected, or immediately if it is not -- until
 * real progress (output produced, or done) happens or there is
 * genuinely nothing left to do right now. */
static void
magnus_h2_proxy_stream_compress_response(struct magnus_h2_stream *stream)
{
    if (stream->io_length > stream->io_sent) return; /* backpressure */
    for (;;) {
        bool complete_by_length = stream->has_response_length
        ...
        if (stream->proxy_stream_compress_inbuf_sent
            ...
            if (received > 0) {
                ...
                stream->last_activity = time(NULL);
                if (stream->cache_this_response_cacheable)
                    magnus_h2_proxy_cache_capture(stream,
                        (const char *) stream->proxy_stream_compress_inbuf,
                        (size_t) received);
            } else if (received == 0) {
                stream->upstream_eof = true;
            } else if (errno == EINTR) {
...

        {
            size_t consumed = 0, produced = 0;
            bool done = false;
            bool ok = magnus_stream_compress_step(stream->proxy_stream_compress,
                stream->proxy_stream_compress_inbuf
                    + stream->proxy_stream_compress_inbuf_sent,
                stream->proxy_stream_compress_inbuf_length
                    - stream->proxy_stream_compress_inbuf_sent,
                complete_by_length || stream->upstream_eof, &consumed,
                (unsigned char *) stream->io_buffer, MAGNUS_PROXY_BUFFER,
                &produced, &done);
            if (!ok) {
                magnus_h2_proxy_abort(stream);
                return;
            }
            stream->proxy_stream_compress_inbuf_sent += consumed;
            if (stream->proxy_stream_compress_inbuf_sent
                == stream->proxy_stream_compress_inbuf_length) {
                stream->proxy_stream_compress_inbuf_length = 0;
                stream->proxy_stream_compress_inbuf_sent = 0;
            }
            stream->io_length = produced;
            stream->io_sent = 0;
            if (done) {
                magnus_stream_compress_end(stream->proxy_stream_compress);
                stream->proxy_stream_compress = NULL;
            ...
            if (produced > 0) return; /* let nghttp2 pull this chunk first */
            /* produced == 0, !done: zero-progress guard -- loop back
             * around, either to fetch more input (if the staging buffer
             * is now empty and more is expected) or to retry step()
             * immediately against whatever is still unconsumed. */
        }
    }
}
...
magnus_h2_handle_upstream(struct magnus_h2_stream *stream, uint32_t flags)
{
    ...
    if (!stream->headers_received) {
        ...
        if (stream->proxy_stream_compress != NULL)
            magnus_h2_proxy_stream_compress_response(stream);
        else
...
magnus_build_metrics(char *out, size_t out_capacity)
{
    ...
    written = (size_t) snprintf(out, out_capacity,
        ...
        "# TYPE magnus_quic_migration_total counter\n"
        ...
        magnus_quic_retry_total(), magnus_quic_migration_total());
...
#undef MAGNUS_FASTCGI_ADD_NV
    ...
    /* Roadmap 5a-4: always requests FCGI_KEEP_CONN -- the application
     * server keeping the connection open after FCGI_END_REQUEST is a
     * precondition for magnus_fastcgi_finish()'s own pool-checkin to
     * ever find a live connection worth pooling; one that ignores this
     * (or closes for its own unrelated reason) is simply caught by
     * magnus_fastcgi_pool_checkout()'s own staleness check at reuse
     * time, so requesting it unconditionally is always safe, not just
     * when pooling this particular endpoint actually pans out. */
    magnus_fastcgi_write_begin_request_body(out + offset,
        MAGNUS_FASTCGI_ROLE_RESPONDER, MAGNUS_FASTCGI_KEEP_CONN);
    offset += 8;
...
/* Roadmap 5a-3/5a-4: common state setup once a socket (freshly
 * connected, or handed out of magnus_fastcgi_pool -- roadmap 5a-4) is
 * ready to be this connection's active FastCGI upstream: resets every
 * per-attempt fastcgi_* field (including fastcgi_out_sent back to 0,
 * so this always resends the whole pre-built request from the start,
 * a fresh BEGIN_REQUEST/PARAMS/STDIN cycle regardless of whether the
 * fd itself is new or reused -- FastCGI's own request_id framing,
 * always 1 here, does not multiplex more than one request per
 * connection at a time either way) and registers it with epoll.
 * `connected` is true immediately for a pooled fd (already established
 * by definition) or a fresh connect() that happened to complete
 * synchronously; otherwise EPOLLOUT will report completion later. The
 * exact shape magnus_proxy_attach_upstream() already established for
 * the identical reason. Returns 0 once the attempt is in flight, -1 on
 * immediate failure (fd already closed by the time this returns -1). */
static int
magnus_fastcgi_attach_upstream(int epoll_fd, magnus_connection_t *connection,
                               size_t endpoint_index, int fd, bool connected,
                               unsigned requests_served)
{
    struct epoll_event event;
    ...
    connection->fastcgi_upstream_connected = connected;
    connection->fastcgi_active = true;
    connection->fastcgi_endpoint_index = endpoint_index;
    connection->fastcgi_upstream_requests_served = requests_served;
...
/* Roadmap 5a-3/5a-4: a pooled idle connection for this endpoint is
 * tried first -- skipping the TCP handshake entirely -- before falling
 * back to opening a fresh one, exactly as magnus_proxy_connect_
 * endpoint() already does for the ordinary HTTP/1.x proxy path.
 * Reuses connection->fastcgi_out (already built by the caller -- see
 * magnus_fastcgi_build_request()); safe to call again for a retry once
 * the previous attempt's upstream has been torn down via magnus_
 * fastcgi_teardown_upstream(). Returns 0 once the attempt is in
 * flight, -1 on immediate failure (the caller records the failure and
                                ...
                                size_t endpoint_index)
{
    ...
    int result;
    unsigned pooled_requests_served;
    int pooled_fd = magnus_fastcgi_pool_checkout(endpoint_index,
                                                 &pooled_requests_served);

    if (pooled_fd >= 0) {
        if (magnus_fastcgi_attach_upstream(epoll_fd, connection,
                                           endpoint_index, pooled_fd, true,
                                           pooled_requests_served) == 0)
            return 0;
        /* attach itself failed (epoll_ctl) -- the fd is already closed;
         * fall through to a fresh connection rather than giving up the
         * whole attempt over what the pool offered. */
    }
    ...
    return magnus_fastcgi_attach_upstream(epoll_fd, connection, endpoint_index,
                                          fd, result == 0, 0);
}
...
/* Roadmap 5a-1/5a-4: translates the CGI-shaped response now fully
 ...
 * drain loop to actually send. The upstream leg either goes back into
 * magnus_fastcgi_pool (cleanly finished, with nothing left over in
 * fastcgi_in past the record stream -- the same "poolable" signal
 * magnus_proxy_flush()'s own identical branch uses its own
 * complete_by_length for) or gets torn down exactly as before this
 * pool existed; independently, the client leg either stays open for
 * its next request or closes, per fastcgi_client_wants_close. A
 * malformed response (no header/body boundary ever found, or an
 * unparseable Status: line) is answered with a clean 502, exactly like
 * any other decode failure this far still gets one (nothing has
...
magnus_fastcgi_finish(int epoll_fd, magnus_connection_t *connection)
{
    ...
    unsigned status = 200;
    magnus_request_t request = {0};
    bool poolable = connection->fastcgi_upstream_fd >= 0
        && connection->fastcgi_in_length == 0;

    if (poolable) {
        int fd = connection->fastcgi_upstream_fd;
        epoll_ctl(epoll_fd, EPOLL_CTL_DEL, fd, NULL);
        magnus_fastcgi_owner[fd] = NULL;
        connection->fastcgi_upstream_fd = -1;
        magnus_fastcgi_pool_checkin(connection->fastcgi_endpoint_index, fd,
            connection->fastcgi_upstream_requests_served + 1);
        if (connection->fastcgi_endpoint_counted) {
            magnus_cluster_endpoint_end(&magnus_fastcgi_cluster,
                                        connection->fastcgi_endpoint_index);
            connection->fastcgi_endpoint_counted = false;
                               ...
                               uint32_t flags)
{
    ...
    if ((flags & (EPOLLIN | EPOLLRDHUP)) == 0) {
        /* The whole request is now sent -- switch epoll interest away
         * from EPOLLOUT (level-triggered, so it would otherwise keep
         * firing on every epoll_wait() for as long as the socket's own
         * send buffer stays writable, a busy-spin this codebase's
         * proxy-dispatch equivalent already guards against the same
         * way at its own identical point). A fresh connection's
         * request is typically small enough to finish in the very same
         * send() call that first observes it as writable, so this gap
         * was previously invisible -- short-lived enough (this attempt
         * finishes and the fd is torn down or checked in within the
         * same handful of epoll iterations) that the wasted wakeups
         * never accumulated into anything visible. Roadmap 5a-4's own
         * pool changed that: a pooled connection can now sit through
         * many additional epoll iterations across this and later
         * requests, giving a stale EPOLLOUT registration real time to
         * spin. */
        struct epoll_event event = { .events = EPOLLIN | EPOLLRDHUP,
                                     .data.fd = connection->fastcgi_upstream_fd };
        epoll_ctl(epoll_fd, EPOLL_CTL_MOD, connection->fastcgi_upstream_fd,
                 &event);
        return 0;
    }
    for (;;) {
...
/* Roadmap 5a-4: bounds how long a FastCGI attempt may spend connecting
 * to, or waiting on, an upstream -- see MAGNUS_FASTCGI_CONNECT_
 * TIMEOUT_SECONDS/MAGNUS_FASTCGI_READ_TIMEOUT_SECONDS's own doc
 * comment for why this exists at all. Mirrors magnus_expire_proxies()'s
 * own shape closely, with one simplification: magnus_proxy dispatch
 * needs a separate "response already started, must abort uncleanly"
 * case for its own read-timeout branch, but FastCGI dispatch never
 * does (see magnus_fastcgi_fail()'s own doc comment -- nothing ever
 * reaches the client before magnus_fastcgi_finish() runs), so a
 * read-timeout here always answers cleanly via magnus_fastcgi_fail()
 * with no abort() counterpart to choose between. Only the connect-stage
 * timeout retries (magnus_fastcgi_connect_failed()) -- a connected-but-
 * silent upstream is deliberately NOT retried, the same retry-storm/
 * tail-latency-amplification avoidance magnus_expire_proxies()'s own
 * identical asymmetry already documents (the endpoint already accepted
 * the connection; retrying would only double the wait without
 * addressing a genuinely slow responder), though it still counts as a
 * passive-health failure either way. */
static void
magnus_fastcgi_expire(int epoll_fd, time_t now)
{
    int fd;
    for (fd = 0; fd < MAGNUS_MAX_FDS; fd++) {
        magnus_connection_t *connection = magnus_connections[fd];
        int result;
        if (connection == NULL || !connection->fastcgi_active) continue;
        if (!connection->fastcgi_upstream_connected) {
            if (now - connection->fastcgi_connect_started
                < MAGNUS_FASTCGI_CONNECT_TIMEOUT_SECONDS) continue;
            result = magnus_fastcgi_connect_failed(epoll_fd, connection, 504,
                                                   "Gateway Timeout");
        } else if (now - connection->fastcgi_last_activity
                   >= MAGNUS_FASTCGI_READ_TIMEOUT_SECONDS) {
            magnus_cluster_result(&magnus_fastcgi_cluster,
                                  connection->fastcgi_endpoint_index, false,
                                  magnus_now_ms());
            result = magnus_fastcgi_fail(epoll_fd, connection, 504,
                                         "Gateway Timeout");
        } else {
            continue;
        }
...
/* Roadmap 5b-1 (SCGI dispatch): tears down just the current connect/
 * upstream attempt -- fd, epoll registration, and the cluster's own
 * concurrent-request counter -- mirrors magnus_fastcgi_teardown_
 * upstream()'s own shape, simplified for SCGI's own no-retry-yet scope
 * (there is no fastcgi_in-equivalent buffer to drop here in the first
 * place -- see scgi_stdout's own doc comment on why response bytes
 * never need a separate not-yet-parsed staging buffer). */
static void
magnus_scgi_teardown_upstream(int epoll_fd, magnus_connection_t *connection)
{
    if (connection->scgi_upstream_fd >= 0) {
        epoll_ctl(epoll_fd, EPOLL_CTL_DEL, connection->scgi_upstream_fd, NULL);
        magnus_scgi_owner[connection->scgi_upstream_fd] = NULL;
        close(connection->scgi_upstream_fd);
        connection->scgi_upstream_fd = -1;
    }
    connection->scgi_active = false;
    if (connection->scgi_endpoint_counted) {
        magnus_cluster_endpoint_end(&magnus_scgi_cluster,
                                    connection->scgi_endpoint_index);
        connection->scgi_endpoint_counted = false;
    }
}

/* Roadmap 5b-1: ends an SCGI attempt before any response bytes have
 * reached the client -- the same "buffer everything, so every failure
 * is still a clean, synthesizable status code" property magnus_
 * fastcgi_fail()'s own doc comment describes, true here for the
 * identical reason (nothing is relayed to the client until magnus_
 * scgi_finish() runs). This first cut has no retry yet (see MAGNUS_
 * ROUTE_ACTION_SCGI's own doc comment), so unlike magnus_fastcgi_fail()
 * this is the *only* way an SCGI attempt ever ends unsuccessfully --
 * there is no separate magnus_scgi_connect_failed() to route through
 * first. */
static int
magnus_scgi_fail(int epoll_fd, magnus_connection_t *connection,
                 unsigned status, const char *reason)
{
    magnus_request_t request = {0};
    magnus_scgi_teardown_upstream(epoll_fd, connection);
    free(connection->scgi_out);
    connection->scgi_out = NULL;
    connection->scgi_out_length = 0;
    connection->scgi_out_sent = 0;
    memcpy(request.request_id, connection->scgi_request_id,
          sizeof(request.request_id));
    magnus_prepare_response(connection, status, reason, "text/plain",
                            status == 504 ? "gateway timeout\n"
                                          : "bad gateway\n",
                            false, true, &request);
    {
        ...
        magnus_record_latency(latency_ms);
        magnus_access_log(request.request_id, connection->client_address,
                          connection->scgi_log_method,
                          connection->scgi_log_target, status, latency_ms,
                          -1);
    }
    return magnus_update_interest(epoll_fd, connection, EPOLLOUT | EPOLLRDHUP);
}

/* Roadmap 5b-1: builds the whole outbound SCGI byte stream -- the
 * netstring-framed header block, then the raw request body immediately
 * after (any body was already fully buffered into connection->body
 * ahead of dispatch, the same generic pre-dispatch buffering
 * action=proxy/action=fastcgi already share) -- into connection->
 * scgi_out, ready to send whole once connected. CONTENT_LENGTH is
 * written first (the one genuinely mandatory ordering constraint the
 * SCGI spec itself imposes), SCGI=1 second (conventional, declares
 * protocol-version support), the rest of the CGI metavariable set
 * following in the same order magnus_fastcgi_build_request() already
 * uses for its own PARAMS record, including DOCUMENT_ROOT (SCGI has no
 * single standardized "how does the application resolve a script path"
 * convention the way FastCGI's own SCRIPT_FILENAME/nginx pairing does,
 * so DOCUMENT_ROOT + SCRIPT_NAME, the same pair CGI itself always
 * provided, is what this codebase forwards and leaves the application
 * to resolve itself). Returns false on any allocation/encoding failure,
 * leaving connection untouched (scgi_out stays NULL, connection->body
 * untouched) -- the caller answers 502, same graceful-failure contract
 * every other dispatch path here already has. */
static bool
magnus_scgi_build_request(magnus_connection_t *connection,
                          const magnus_http_request_t *parsed,
                          const char *forward_path)
{
    char headers[4096];
...
#define MAGNUS_SCGI_ADD_NV(nv_name, nv_value) \
    do { \
        size_t nv_len = magnus_scgi_encode_nv((nv_name), (nv_value), \
            headers + headers_length, sizeof(headers) - headers_length); \
        if (nv_len == 0) return false; \
        headers_length += nv_len; \
    } while (0)

    MAGNUS_SCGI_ADD_NV("CONTENT_LENGTH", content_length_text);
    MAGNUS_SCGI_ADD_NV("SCGI", "1");
    MAGNUS_SCGI_ADD_NV("REQUEST_METHOD", parsed->method);
    MAGNUS_SCGI_ADD_NV("SCRIPT_NAME", script_name);
    MAGNUS_SCGI_ADD_NV("QUERY_STRING", query_string);
    MAGNUS_SCGI_ADD_NV("REQUEST_URI", forward_path);
    MAGNUS_SCGI_ADD_NV("DOCUMENT_ROOT", magnus_scgi_root);
    MAGNUS_SCGI_ADD_NV("SERVER_PROTOCOL", "HTTP/1.1");
    MAGNUS_SCGI_ADD_NV("SERVER_SOFTWARE", "Magnus/" MAGNUS_VERSION);
    MAGNUS_SCGI_ADD_NV("SERVER_NAME",
                       parsed->host[0] != '\0' ? parsed->host : "localhost");
    MAGNUS_SCGI_ADD_NV("SERVER_PORT", server_port);
    MAGNUS_SCGI_ADD_NV("REMOTE_ADDR", remote_addr);
    if (content_type != NULL) MAGNUS_SCGI_ADD_NV("CONTENT_TYPE", content_type);
    MAGNUS_SCGI_ADD_NV("GATEWAY_INTERFACE", "CGI/1.1");
    if (connection->tls != NULL) MAGNUS_SCGI_ADD_NV("HTTPS", "on");

#undef MAGNUS_SCGI_ADD_NV

    prefix_length = magnus_scgi_write_netstring_prefix(headers_length, prefix,
                                                        sizeof(prefix));
    if (prefix_length == 0) return false;

    total = prefix_length + headers_length + 1 /* ',' */
        + connection->body_length;
    out = malloc(total);
    if (out == NULL) return false;

    memcpy(out, prefix, prefix_length);
    memcpy(out + prefix_length, headers, headers_length);
    out[prefix_length + headers_length] = ',';
    if (connection->body_length > 0)
        memcpy(out + prefix_length + headers_length + 1, connection->body,
              connection->body_length);

    connection->scgi_out = out;
    connection->scgi_out_length = total;
    connection->scgi_out_sent = 0;
...
/* Roadmap 5b-1: common state setup once a freshly connected socket is
 * ready to be this connection's active SCGI upstream -- mirrors
 * magnus_fastcgi_attach_upstream()'s own shape, simplified for this
 * first cut's own no-pooling scope (always a fresh connection, never
 * handed out of a pool, so there is no `connected`/`requests_served`
 * parameter to thread through). Returns 0 once the attempt is in
 * flight, -1 on immediate failure (fd already closed by the time this
 * returns -1). */
static int
magnus_scgi_attach_upstream(int epoll_fd, magnus_connection_t *connection,
                            size_t endpoint_index, int fd, bool connected)
{
    struct epoll_event event;

    connection->scgi_upstream_fd = fd;
    connection->scgi_upstream_connected = connected;
    connection->scgi_active = true;
    connection->scgi_endpoint_index = endpoint_index;
    magnus_cluster_endpoint_begin(&magnus_scgi_cluster, endpoint_index);
    connection->scgi_endpoint_counted = true;
    connection->scgi_connect_started = time(NULL);
    connection->scgi_last_activity = connection->scgi_connect_started;
    connection->scgi_out_sent = 0;
    magnus_scgi_owner[fd] = connection;
    event = (struct epoll_event) { .events = EPOLLOUT | EPOLLRDHUP,
                                   .data.fd = fd };
    if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, fd, &event) < 0) {
        magnus_scgi_owner[fd] = NULL;
        close(fd);
        connection->scgi_upstream_fd = -1;
        connection->scgi_active = false;
        return -1;
...
/* Roadmap 5b-1: opens a fresh non-blocking connection to the given
 * cluster endpoint -- no pooled-connection fast path yet (see MAGNUS_
 * ROUTE_ACTION_SCGI's own doc comment), unlike magnus_fastcgi_connect_
 * endpoint()'s own pool-first shape. Returns 0 once the attempt is in
 * flight, -1 on immediate failure. */
static int
magnus_scgi_connect_endpoint(int epoll_fd, magnus_connection_t *connection,
                             size_t endpoint_index)
{
    struct sockaddr_in address;
    int fd;
    ...
    address.sin_family = AF_INET;
    address.sin_port = htons(
        (uint16_t) magnus_scgi_cluster.endpoints[endpoint_index].port);
    if (inet_pton(AF_INET, magnus_scgi_cluster.endpoints[endpoint_index].address,
                 &address.sin_addr) != 1)
        return -1;
    fd = socket(AF_INET, SOCK_STREAM | SOCK_NONBLOCK | SOCK_CLOEXEC, 0);
    if (fd < 0 || fd >= MAGNUS_MAX_FDS) {
    ...
    return magnus_scgi_attach_upstream(epoll_fd, connection, endpoint_index,
                                       fd, result == 0);
}

/* Roadmap 5b-1: dispatches a matched action=scgi route. Builds the
 * whole SCGI request up front (magnus_scgi_build_request()) and opens
 * a non-blocking connection to the cluster's own next round-robin
 * endpoint. Returns 0 once that connect attempt (and the request
 * buffer to send once it completes) is in flight -- magnus_scgi_
 * handle_upstream() drives the rest, the same "asynchronous from here"
 * contract every other dispatch path in this codebase already has.
 * Returns -1 on any failure -- no cluster endpoint available, the
 * request buffer failing to build, or the connect attempt itself
 * failing immediately -- never partially started; there is no retry
 * yet to fall back to (see MAGNUS_ROUTE_ACTION_SCGI's own doc
 * comment), so any failure here is final, the caller answers 502. */
static int
magnus_scgi_pick_and_start(int epoll_fd, magnus_connection_t *connection,
                           magnus_request_t *request,
                           const magnus_http_request_t *parsed,
                           const char *forward_path, bool client_wants_close)
{
    int endpoint = magnus_cluster_select(&magnus_scgi_cluster, magnus_now_ms(),
                                         NULL, connection->client_address);
    if (endpoint < 0) return -1;
    if (!magnus_scgi_build_request(connection, parsed, forward_path))
        return -1;

    connection->scgi_client_wants_close = client_wants_close;
    memcpy(connection->scgi_request_id, request->request_id,
          sizeof(connection->scgi_request_id));

    if (magnus_scgi_connect_endpoint(epoll_fd, connection,
                                     (size_t) endpoint) == 0)
        return 0;
    magnus_cluster_result(&magnus_scgi_cluster, (size_t) endpoint, false,
                          magnus_now_ms());
    free(connection->scgi_out);
    connection->scgi_out = NULL;
    return -1;
 ...
 * magnus_scgi_handle_upstream()'s own recv() loop). */
static bool
magnus_scgi_append_stdout(magnus_connection_t *connection, const void *data,
                          size_t len)
{
    char *grown;
    ...
    if (connection->scgi_stdout_length + len
        ...
        size_t new_capacity = connection->scgi_stdout_capacity == 0
            ? MAGNUS_PROXY_BUFFER : connection->scgi_stdout_capacity * 2;
        while (new_capacity < connection->scgi_stdout_length + len)
            new_capacity *= 2;
        grown = realloc(connection->scgi_stdout, new_capacity);
        if (grown == NULL) return false;
        connection->scgi_stdout = grown;
        connection->scgi_stdout_capacity = new_capacity;
    }
    memcpy(connection->scgi_stdout + connection->scgi_stdout_length, data,
          len);
    connection->scgi_stdout_length += len;
    return true;
}

/* Roadmap 5b-1: translates the CGI-shaped response now fully
 * accumulated in connection->scgi_stdout into a real HTTP/1.1 response
 * and hands it to magnus_handle_write()'s own scgi_response_headers/
 * scgi_stdout(+scgi_body_offset) drain loop to actually send -- reuses
 * magnus_fastcgi_find_body()/magnus_fastcgi_translate_headers()
 * directly (see magnus_scgi.h's own top comment on why: the response
 * shape is identical CGI framing either way, so there is nothing
 * SCGI-specific left to translate). The upstream leg is always torn
 * down (no pool to check into yet); the client leg either stays open
 * for its next request or closes, per scgi_client_wants_close. A
 * malformed response is answered with a clean 502, exactly like
 * magnus_fastcgi_finish()'s own identical branch, for the identical
 * reason (nothing has reached the client yet). */
static int
magnus_scgi_finish(int epoll_fd, magnus_connection_t *connection)
{
    const char *body;
    size_t header_text_length;
    size_t body_length;
    ...
    body = magnus_fastcgi_find_body(connection->scgi_stdout,
                                    connection->scgi_stdout_length,
                                    &header_text_length);
    if (body == NULL)
        return magnus_scgi_fail(epoll_fd, connection, 502, "Bad Gateway");
    body_length = connection->scgi_stdout_length
        - (size_t) (body - connection->scgi_stdout);

    headers = malloc(MAGNUS_PROXY_SANITIZED_LIMIT);
    if (headers == NULL)
        return magnus_scgi_fail(epoll_fd, connection, 502, "Bad Gateway");
    headers_length = magnus_fastcgi_translate_headers(
        connection->scgi_stdout, header_text_length, body_length,
        connection->scgi_client_wants_close, NULL, "magnus-scgi/0.1",
        headers, MAGNUS_PROXY_SANITIZED_LIMIT, &status);
    if (headers_length < 0) {
        free(headers);
        return magnus_scgi_fail(epoll_fd, connection, 502, "Bad Gateway");
    ...
    connection->scgi_response_headers = headers;
    connection->scgi_response_headers_length = (size_t) headers_length;
    connection->scgi_response_headers_sent = 0;
    connection->scgi_body_offset = (size_t) (body - connection->scgi_stdout);
    connection->scgi_body_sent = 0;
    connection->close_after_write = connection->scgi_client_wants_close;
...
/* Roadmap 5b-1: entry point from the main epoll loop for any event on
 * connection->scgi_upstream_fd -- drives connect completion, sending
 * the whole pre-built request (magnus_scgi_build_request()), and
 * receiving the response, the same three-stage shape magnus_fastcgi_
 * handle_upstream() already has. Unlike FastCGI's own record-framed
 * stream (which knows a response is complete once FCGI_END_REQUEST is
 * seen), SCGI has no such marker at all -- the *only* completion
 * signal the protocol itself ever provides is the upstream closing the
 * connection, so recv() returning 0 is always treated as a clean
 * finish here, never a truncation (as long as at least the CGI header/
 * body boundary has actually appeared in what was received --
 * magnus_scgi_finish() itself is what catches an EOF before that ever
 * happened, via magnus_fastcgi_find_body() returning NULL). */
static int
magnus_scgi_handle_upstream(int epoll_fd, magnus_connection_t *connection,
                            uint32_t flags)
{
    if ((flags & (EPOLLERR | EPOLLHUP)) != 0) {
        magnus_cluster_result(&magnus_scgi_cluster,
                              connection->scgi_endpoint_index, false,
                              magnus_now_ms());
        return magnus_scgi_fail(epoll_fd, connection, 502, "Bad Gateway");
    }
    if (!connection->scgi_upstream_connected) {
        int error = 0;
        socklen_t length = sizeof(error);
        if (getsockopt(connection->scgi_upstream_fd, SOL_SOCKET, SO_ERROR,
                       &error, &length) < 0 || error != 0) {
            magnus_cluster_result(&magnus_scgi_cluster,
                                  connection->scgi_endpoint_index, false,
                                  magnus_now_ms());
            return magnus_scgi_fail(epoll_fd, connection, 504,
                                    "Gateway Timeout");
        }
        connection->scgi_upstream_connected = true;
        connection->scgi_last_activity = time(NULL);
    }
    while (connection->scgi_out_sent < connection->scgi_out_length) {
        ssize_t sent = send(connection->scgi_upstream_fd,
            connection->scgi_out + connection->scgi_out_sent,
            connection->scgi_out_length - connection->scgi_out_sent,
            MSG_NOSIGNAL);
        if (sent > 0) {
            connection->scgi_out_sent += (size_t) sent;
            connection->scgi_last_activity = time(NULL);
        } else if (sent < 0 && errno == EINTR) {
            continue;
        } else if (sent < 0 && (errno == EAGAIN || errno == EWOULDBLOCK)) {
    ...
    if ((flags & (EPOLLIN | EPOLLRDHUP)) == 0) {
        /* Same EPOLLOUT->EPOLLIN demotion magnus_fastcgi_handle_
         * upstream()'s own identical block documents in full -- applied
         * here from the very first cut rather than found as a later
         * regression, since it is already a known real bug class in
         * this exact codebase, not a hypothetical one. */
        struct epoll_event event = { .events = EPOLLIN | EPOLLRDHUP,
                                     .data.fd = connection->scgi_upstream_fd };
        epoll_ctl(epoll_fd, EPOLL_CTL_MOD, connection->scgi_upstream_fd,
                 &event);
        return 0;
    }
    for (;;) {
        char buffer[MAGNUS_PROXY_BUFFER];
        ssize_t received = recv(connection->scgi_upstream_fd, buffer,
                                sizeof(buffer), 0);
        if (received > 0) {
            connection->scgi_last_activity = time(NULL);
            if (!magnus_scgi_append_stdout(connection, buffer,
                                           (size_t) received)) {
                magnus_cluster_result(&magnus_scgi_cluster,
                                      connection->scgi_endpoint_index, false,
                                      magnus_now_ms());
                return magnus_scgi_fail(epoll_fd, connection, 502,
                                        "Bad Gateway");
            }
            continue;
        }
        if (received == 0) {
            magnus_cluster_result(&magnus_scgi_cluster,
                                  connection->scgi_endpoint_index, true,
                                  magnus_now_ms());
            return magnus_scgi_finish(epoll_fd, connection);
        }
        if (errno == EINTR) continue;
        if (errno == EAGAIN || errno == EWOULDBLOCK) return 0;
        magnus_cluster_result(&magnus_scgi_cluster,
...
/* Roadmap 5b-1: bounds how long an SCGI attempt may spend connecting
 * to, or waiting on, an upstream -- see MAGNUS_SCGI_CONNECT_TIMEOUT_
 * SECONDS's own doc comment for why this is already here in this first
 * cut. Mirrors magnus_fastcgi_expire()'s own shape exactly, minus the
 * retry call (no magnus_scgi_connect_failed() exists yet -- a connect-
 * stage timeout here is simply a final magnus_scgi_fail()). */
static void
magnus_scgi_expire(int epoll_fd, time_t now)
{
    int fd;
    for (fd = 0; fd < MAGNUS_MAX_FDS; fd++) {
        magnus_connection_t *connection = magnus_connections[fd];
        int result;
        if (connection == NULL || !connection->scgi_active) continue;
        if (!connection->scgi_upstream_connected) {
            if (now - connection->scgi_connect_started
                < MAGNUS_SCGI_CONNECT_TIMEOUT_SECONDS) continue;
            magnus_cluster_result(&magnus_scgi_cluster,
                                  connection->scgi_endpoint_index, false,
                                  magnus_now_ms());
            result = magnus_scgi_fail(epoll_fd, connection, 504,
                                      "Gateway Timeout");
        } else if (now - connection->scgi_last_activity
                   >= MAGNUS_SCGI_READ_TIMEOUT_SECONDS) {
            magnus_cluster_result(&magnus_scgi_cluster,
                                  connection->scgi_endpoint_index, false,
                                  magnus_now_ms());
            result = magnus_scgi_fail(epoll_fd, connection, 504,
                        ...
                        const magnus_http_request_t *parsed)
{
    ...
    if (decision.route_denied) {
        ...
        if (start_result == 0) {
             ...
             * above -- the request completes asynchronously, once the
             * upstream responds (magnus_scgi_finish()/_fail()). */
            strncpy(connection->scgi_log_method, request.method,
                   sizeof(connection->scgi_log_method) - 1);
            connection->scgi_log_method[
                sizeof(connection->scgi_log_method) - 1] = '\0';
            strncpy(connection->scgi_log_target, request.path,
...
magnus_handle_write(int epoll_fd, magnus_connection_t *connection)
{
    ...
    /* SCGI dispatch (roadmap 5b-1): same shape as the FastCGI drain
     * loops just above. */
    while (connection->scgi_response_headers != NULL
           && connection->scgi_response_headers_sent
              < connection->scgi_response_headers_length) {
        sent = magnus_socket_write(connection,
            connection->scgi_response_headers
                + connection->scgi_response_headers_sent,
            connection->scgi_response_headers_length
                - connection->scgi_response_headers_sent);
        if (sent > 0) {
            connection->scgi_response_headers_sent += (size_t) sent;
            magnus_bytes_sent += (uint64_t) sent;
            connection->last_active = time(NULL);
            continue;
        }
    ...
    while (connection->scgi_response_headers != NULL
           && connection->scgi_body_sent
              < connection->scgi_stdout_length - connection->scgi_body_offset) {
        sent = magnus_socket_write(connection,
            connection->scgi_stdout + connection->scgi_body_offset
                + connection->scgi_body_sent,
            connection->scgi_stdout_length - connection->scgi_body_offset
                - connection->scgi_body_sent);
        if (sent > 0) {
            connection->scgi_body_sent += (size_t) sent;
            magnus_bytes_sent += (uint64_t) sent;
            connection->last_active = time(NULL);
            continue;
        }
    ...
    connection->fastcgi_body_sent = 0;
    free(connection->scgi_response_headers);
    connection->scgi_response_headers = NULL;
    connection->scgi_response_headers_length = 0;
    connection->scgi_response_headers_sent = 0;
    free(connection->scgi_stdout);
    connection->scgi_stdout = NULL;
    connection->scgi_stdout_length = 0;
...
magnus_apply_config(const magnus_config_t *config)
{
    ...
    /* SCGI dispatch (roadmap 5b-1): same round_robin-only,
     * no-configurable-policy scope as the FastCGI cluster's own
     * identical comment just above. */
    magnus_cluster_init(&new_scgi_cluster,
                        config->health_check_failure_threshold,
                        (uint64_t) config->health_check_cooldown_seconds
                        * 1000, MAGNUS_LB_ROUND_ROBIN);
    for (index = 0; index < config->scgi_upstream_count; index++) {
        if (magnus_cluster_add(&new_scgi_cluster,
    ...
    /* Same stale-by-position hazard as magnus_pool_close_all()/
     * magnus_grpc_pool_close_all() above, for FastCGI's own connection
     * pool (roadmap 5a-4) -- flush it before swapping in the new
     * cluster, not after. */
    magnus_fastcgi_pool_close_all();
    ...
    /* SCGI dispatch (roadmap 5b-1): no connection pool to flush yet
     * (see MAGNUS_ROUTE_ACTION_SCGI's own doc comment) -- a straight
     * swap-in, same as the gRPC/FastCGI clusters' own count/root
     * updates just above. */
...
magnus_parse_options(int argc, char **argv)
{
    ...
    for (index = 1; index < argc; index += 2) {
        ...
        if (strcmp(argv[index], "--port") == 0) {
        ...
        } else if (strcmp(argv[index], "--scgi-upstream") == 0) {
            /* ipv4:port or ipv4:port:weight; repeatable to build the
             * SCGI cluster. Literal IPv4 only, same restriction and
             * reasoning as --fastcgi-upstream's own identical comment. */
            char spec[80];
            char *saveptr = NULL;
            char *address;
            char *port_text;
            ...
            if (magnus_cluster_add(&magnus_scgi_cluster, address,
                                   (unsigned) upstream_port,
                                   (unsigned) weight) != 0) break;
            magnus_scgi_upstream_enabled = true;
        } else if (strcmp(argv[index], "--scgi-root") == 0) {
            struct stat metadata;
            if (strlen(argv[index + 1]) >= sizeof(magnus_scgi_root)) break;
            if (stat(argv[index + 1], &metadata) != 0
                || !S_ISDIR(metadata.st_mode)) break;
    ...
    for (size_t r = 0; r < magnus_route_count; r++) {
        ...
        if (magnus_routes[r].action == MAGNUS_ROUTE_ACTION_SCGI
            && !magnus_scgi_upstream_enabled) {
            fprintf(stderr, "magnus: --route: a route with action=scgi "
...
main(int argc, char **argv)
{
    ...
    while (magnus_running) {
        ...
        for (index = 0; index < ready; index++) {
            ...
            if (fd >= 0 && fd < MAGNUS_MAX_FDS
                && magnus_scgi_owner[fd] != NULL) {
                /* Same shape as the FastCGI branch just above. */
                connection = magnus_scgi_owner[fd];
                result = magnus_scgi_handle_upstream(epoll_fd, connection,
                                                     flags);
                if (result < 0
                    && magnus_connections[connection->fd] != NULL)
                    magnus_close_connection(epoll_fd, connection);
        ...
        if (now != last_sweep) {
            ...
            magnus_fastcgi_pool_expire_idle(now);
