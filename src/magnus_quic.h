 ...
 *     protocol, not a QUIC-specific gap); Brotli/zstd; streaming/
 *     chunked compression for files above 2a's own 8 MiB bound
 *   - upstream connection pooling, session affinity, and response
 *     caching for proxy dispatch (4d/4f/4g all still only ever end up
 *     with one *successful* connection against one selected endpoint
 *     of the single shared magnus_cluster -- 4g's own retry only
 *     covers *failed* attempts, bounded by
 *     MAGNUS_QUIC_PROXY_MAX_ATTEMPTS, the same shape
 *     magnus_h2_proxy_connect_failed()'s own retry already has; a
 *     matched action=proxy route decides *whether* and *what path* to
 *     forward, never a different upstream of its own, matching
 *     magnus_proxy_pick_and_start()'s identical h1/h2 behavior; a
 *     route's own `cache_enabled` is deliberately not consulted yet)
 *   - Real-IP-aware (trusted-proxy-resolved) source_cidr route
 *     matching (4f matches against the raw QUIC peer address only --
 *     QUIC has no established PROXY-protocol-over-UDP precedent in
 *     this codebase to resolve a trusted client address from in the
 *     first place)
