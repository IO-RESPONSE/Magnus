 ...
 * dispatch) -- a UDP listener wired into Magnus's own epoll reactor
 * that completes a real ngtcp2 handshake using the ngtcp2 +
 ...
 *   - upstream connection pooling and response caching for proxy
 *     dispatch (4d/4f/4g/4h all still only ever end up with one
 *     *successful* connection against one selected endpoint of the
 *     single shared magnus_cluster -- 4g's own retry only covers
 ...
 *     behavior; a route's own `cache_enabled` is deliberately not
 *     consulted yet)
