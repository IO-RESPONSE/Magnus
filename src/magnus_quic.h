...
/* Phase 4 (roadmap): QUIC transport (4a) + HTTP/3 (4b static files, 4c
 * /healthz//metrics, 4d "/proxy" dispatch, 4e static-file gzip
 * compression, 4f `route` table dispatch, 4g retry-on-connect-failure
 * for proxy dispatch, 4h cookie-based session affinity for proxy
 * dispatch, 4i reverse-proxy response caching for proxy dispatch) -- a
 * UDP listener wired into Magnus's own epoll reactor that completes a
 * real ngtcp2 handshake using the ngtcp2 + libngtcp2_crypto_ossl +
 * nghttp3 stack chosen in
 ...
 *     an unmatched Initial is accepted unconditionally (4a)
 *   - connection migration / path validation beyond what a single,
 *     non-migrating handshake needs (4a)
 *   - 0-RTT (4a)
 ...
 *     and HTTP/2 -- proxied responses stay uncompressed on every
 *     protocol, not a QUIC-specific gap; a *cached* response reuses
 *     whatever the origin itself sent, uncompressed here too, same as
 *     h1/h2); Brotli/zstd; streaming/chunked compression for files
 *     above 2a's own 8 MiB bound
 *   - upstream connection pooling for proxy dispatch (4d/4f/4g/4h/4i
 *     all still only ever end up with one *successful* connection
 *     against one selected endpoint of the single shared
 *     magnus_cluster per request -- 4g's own retry only covers
 *     *failed* attempts, bounded by MAGNUS_QUIC_PROXY_MAX_ATTEMPTS,
 *     the same shape magnus_h2_proxy_connect_failed()'s own retry
 *     already has; a matched action=proxy route decides *whether* and
 *     *what path* to forward, never a different upstream of its own,
 *     matching magnus_proxy_pick_and_start()'s identical h1/h2
 *     behavior)
