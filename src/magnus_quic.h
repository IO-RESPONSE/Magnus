 ...
 * dispatch, 4i reverse-proxy response caching for proxy dispatch, 4j
 * upstream connection pooling for proxy dispatch) -- a UDP listener
 * wired into Magnus's own epoll reactor that completes a real ngtcp2
 * handshake using the ngtcp2 + libngtcp2_crypto_ossl + nghttp3 stack
 * chosen in docs/phase4-http3-quic-dependency-evaluation.md and
 * verified working against this host's OpenSSL in
 * docs/phase4-spike-results.md, then serves real HTTP/3 traffic over
 * it -- src/magnus_quic.c's own section comments (search "roadmap 4")
 * have each sub-phase's exact scope.
 *
